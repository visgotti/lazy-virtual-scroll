...
const LazyVirtualList: React.FC<VirtualListProps> = ({
  ...
  const clientLengthProp = direction === 'column' ? 'clientHeight' : 'clientWidth';
  const lengthProp = direction === 'column' ? 'height' : 'width';
  const scrollProp = direction === 'column' ? 'scrollTop' : 'scrollLeft';
  const marginProp = direction === 'column' ? 'marginTop' : 'marginLeft';
  const marginProp2 = direction === 'column' ? 'marginBottom' : 'marginRight';
...
const finalArray = useMemo(() => {
  ...
  return (
    <div className="scroll-outer" ref={handleScrollOuterRef} style={{ [`max${capitalize(lengthProp)}`]: '100%', [`min${capitalize(lengthProp)}`]: '100%' }}>
      <div className="scroll-inner" ref={scrollInnerRef} style={{ ['flexDirection']: direction, [lengthProp]: `${scrollLength}px`, [`max${capitalize(lengthProp)}`]: `${scrollLength}px`, [`min${capitalize(lengthProp)}`]: `${scrollLength}px`, [`${marginProp}`]: `${scrollMargin}px` }}>
